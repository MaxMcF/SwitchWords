...
class Node:
    def __init__(self, word_id, next_n=None, prev_n=None):
...
#     def __init__(self, length,  word_id, next_n=None, prev_n=None):
#         super().__init__(word_id, next_n=None, prev_n=None)
#         self.length = length
...
class Word:
    def __init__(self, length):
        ...
        for i in range(length):
            new_node = Node(word_id=self.word_id, next_n=prev)
            if prev:
...
class Board:
    ...
    def populate_board(self):
        base = self.words[0]
        valid_words = self.query_db(f'SELECT * FROM words WHERE length(word) = {base.length}')
        ...
        for word in self.words[1:]:
            ...
            self.populate_word(head, new_word[0])
